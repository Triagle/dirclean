...
module Rule = struct
  ...
  let rec rule_of_ini path base_rule = function
    ...
    | ("move_to", String folder)::xs -> rule_of_ini path {base_rule with action = Some (file_move folder)} xs
  ...
  let execute_rule = function
    | {path = Some path; selector = Some selector; action = Some action} when Sys.is_directory path ->
